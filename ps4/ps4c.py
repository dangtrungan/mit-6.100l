...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        ...
        word_count = sum(1 for word in words if is_word(dictionary, word))

        if highest_word_count <= word_count:
            highest_word_count = word_count
            best_decrypted_text = plaintext
...
def decode_story():
    ...
    '''
    return decrypt_message_try_pads(ps4b.EncryptedMessage(get_story_string()), get_story_pads()).get_text()
